...
var (
	competitionColumns               = []string{"id", "name", "type", "external_entity_id", "created_at", "updated_at"}
	competitionColumnsWithoutDefault = []string{"name", "type", "external_entity_id", "created_at", "updated_at"}
...
func (o *Competition) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `competitions` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `competitions` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, competitionPrimaryKeyColumns))
		}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == competitionMapping["ID"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for competitions")
	}
...
func (o *Competition) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "competitions", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `competitions` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for competitions")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for competitions")
	}
...
func (o CompetitionSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("models: no Competition slice provided for delete all")
	}
